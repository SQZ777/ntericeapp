...
async function get_latest_video_v2() {
    let new_video = get_latest_video();
    let video = await MongoDB.findOneData({
        newest_video_id: new_video.video_id
    });
    console.log(video)
    console.log(new_video)
    if (new_video.videoId !== video.newest_video_id) { //new video is not exist in mongodb
