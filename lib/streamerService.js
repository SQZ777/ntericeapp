...
async function run(client, streamerCollection) {
  ...
  const channels = await streamerService.getStreamers();
  // const channelNames = [
  //   'thisiceisfromtaiwan',
  //   'MorganTang',
  //   'hsiny0903',
  ...
  channels.map(async (channel) => {
    const streamer = await streamerService.getStreamer(channel.name);
    const channelStatusResp = await twitchService.getChannelStatus(channel.name);
    if (streamer.status === 'close' && channelStatusResp !== undefined) {
      const twitchUser = await twitchService.getUser(channelStatusResp.user_id);
      ...
      await streamerService.updateStreamerCloseTime(channel.name);
    }
  });
}
