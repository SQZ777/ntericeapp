...
const { IgotalldayYoutubeRepository } = require('./igotalldayYoutubeRepository');
...
async function run(client, igotalldayCollection) {
  const igotalldayYoutubeRepository = new IgotalldayYoutubeRepository(igotalldayCollection);
  const newVideo = await getLatestVideoFromYoutube();
  if (await igotalldayYoutubeRepository.isVideoLatest(newVideo.videoId)) {
    const currentVideoId = await igotalldayYoutubeRepository.getCurrentVideoId();
    await igotalldayYoutubeRepository.updateLatestVideo(currentVideoId, newVideo.videoId);
